...
prob_i_k_PY <- function(obs_index, clust_index, Ci, data, sig, lambda, alpha, theta, eig, verbose=FALSE){
  ...
  if(!(clust_index%in%Ci)){ # check if it is a new cluster
    ...
    if(length(idx_k)>1){
      ...
      # print(paste("posterior value for clsuter",clust_index,"=",((length(idx_k)-sig)/(N + theta))*exp(-lambda*(disc_in - disc_no_i))))
      # return(((length(idx_k)-sig)/(N + theta))*exp(-lambda*(disc_in - disc_no_i)))
      ...
      # print(paste("posterior value for clsuter",clust_index,"=",((1-sig)/(N + theta))*exp(-lambda*(disc_in - disc_no_i))))
      # return(((1-sig)/(N + theta))*exp(-lambda*(disc_in - disc_no_i))) # approx to be checked with mario
      return((1-sig)*exp(-lambda*(disc_in - disc_no_i)))
    }
  }
}
...
update_k_i <- function(obs_idx, Ci, data, sig, lambda, alpha, theta, eig, verbose){
...
gibbs_sampler <- function(N, N_burnin, x0, data, lambda, alpha, eig, sig, theta, verbose = F){
  ...
  while(posz < N){
    ...
    for(t in 1:n)#update one component at the time
      X_temp[t] = update_k_i(t, X_temp, data, sig, lambda, alpha, theta, eig, verbose)
...
Ci <- gibbs_sampler(5, 0, c_opt_test, data, .5, alpha, eigen(K_1), 0.25, 3.66, verbose = T)

for(t in 1:20)
  print("#############################################")

#actual run
Ci <- gibbs_sampler(10000, 1000, c_opt_test, data, 1e-3, alpha, eigen(K_1), 0.25, 3.66, verbose = T)
