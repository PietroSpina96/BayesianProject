...
update_k_i <- function(obs_idx, Ci, n_clust, data, lambda, alpha, eig){
  # Sample a value for the cluster, compatible with the given expression of the full conditional
  probs = rep(0,n_clust)
  for(ii in 1:n_clust)
    probs[ii] = prob_i_k(obs_idx, ii, Ci, data, lambda, alpha, eig,verbose=F) #FOR UNIFORM
...
gibbs_sampler_uniform <- function(N, N_burnin, x0, data, lambda, alpha, eig, verbose = F){
  ...
  while(posz < N){
    ...
    for(t in 1:n)#update one component at the time
      X_temp[t] = update_k_i(t, X_temp, k, data, lambda, alpha, eig)
...
Ci <- gibbs_sampler_uniform(5, 1, c_opt_2, f.Data$data, 1, 1e4 , eigen(cov(f.Data$data)), verbose = T)



#actual run

Ci <- gibbs_sampler_uniform(10000, 1000, c_opt_2, f.Data$data, 1, 1e4 , eigen(cov(f.Data$data)), verbose = T)



save(Ci, file="indexes_Ci_uniform_lambda1_realData.RData")

save.image("chain_Ci_data1_uniform_realData.RData")
